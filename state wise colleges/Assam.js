...
async function scrapeColleges() {
  ...
  let browser;
  let allColleges = [];
  const totalColleges = 300; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/assam-colleges', {
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/assam-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
