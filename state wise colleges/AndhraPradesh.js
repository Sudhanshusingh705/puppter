...
async function scrapeColleges() {
  ...
  let browser;
  let allColleges = [];
  const totalColleges = 780; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/andhra-pradesh-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'AndhraPradesh_colleges.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/andhra-pradesh-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
