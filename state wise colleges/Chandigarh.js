...
async function scrapeColleges() {
  ...
  let browser;
  let allColleges = [];
  const totalColleges = 100; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/chandigarh-colleges', {
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/chandigarh-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
