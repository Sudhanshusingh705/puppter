...
async function scrapeColleges() {
  ...
  let browser;
  let allColleges = [];
  const totalColleges = 2140; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/uttar-pradesh-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'UttarPradesh_colleges.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/uttar-pradesh-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
