...
async function scrapeColleges() {
  ...
  let browser;
  let allColleges = [];
  const totalColleges = 200; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/himachal-pradesh-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'HimachalPradesh_colleges.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/himachal-pradesh-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
