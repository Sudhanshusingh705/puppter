...
async function scrapeColleges() {
  ...
  let browser;
  let allColleges = [];
  const totalColleges = 770; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/punjab-colleges', {
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/punjab-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
