...
async function scrapeColleges() {
  ...
  let browser;
  let allColleges = [];
  const totalColleges = 280; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/chhattisgarh-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'Chhatishgarh_colleges.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/chhattisgarh-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
