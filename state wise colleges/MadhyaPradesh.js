...
async function scrapeColleges() {
  ...
  let browser;
  let allColleges = [];
  const totalColleges = 850; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/madhya-pradesh-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'MadhyaPradesh_colleges.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/madhya-pradesh-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
