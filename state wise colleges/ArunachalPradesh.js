...
async function scrapeColleges() {
  ...
  let browser;
  let allColleges = [];
  const totalColleges = 50; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/arunachal-pradesh-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'ArunachalPradesh_colleges.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/arunachal-pradesh-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
