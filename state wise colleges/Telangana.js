...
async function scrapeColleges() {
  ...
  let browser;
  let allColleges = [];
  const totalColleges = 760; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/telangana-colleges', {
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/telangana-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
