...
async function scrapeColleges() {
  ...
  let browser;
  let allColleges = [];
  const totalColleges = 10; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/andaman-and-nicobar-islands-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'AndamanNicobar_colleges.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/andaman-and-nicobar-islands-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
