...
async function scrapeColleges() {
  ...
  let browser;
  let allColleges = [];
  const totalColleges = 170; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/jammu-and-kashmir-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'JammuKashmir_colleges.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/jammu-and-kashmir-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
