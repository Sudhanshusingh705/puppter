...
async function scrapeCollegeLogos() {
  ...
  let browser;
  let allLogos = [];
  const totalColleges = 4990; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/computer-applications-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'ComputerApplication_colleges_logos.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/computer-applications-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
