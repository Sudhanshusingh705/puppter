...
async function scrapeCollegeLogos() {
  ...
  let browser;
  let allLogos = [];
  const totalColleges = 513; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/dental-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'Dental_colleges_logos.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/dental-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
