...
async function scrapeCollegeLogos() {
  ...
  let browser;
  let allLogos = [];
  const totalColleges = 1980; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/paramedical-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'Paramedical_colleges_logos.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/paramedical-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
