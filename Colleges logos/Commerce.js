...
async function scrapeCollegeLogos() {
  ...
  let browser;
  let allLogos = [];
  const totalColleges = 4580; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/commerce-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'Commerce_colleges_logos.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/commerce-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
