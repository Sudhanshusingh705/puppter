...
async function scrapeCollegeLogos() {
  ...
  let browser;
  let allLogos = [];
  const totalColleges = 1450; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/mass-communications-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'MassCommunication_colleges_logos.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/mass-communications-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
