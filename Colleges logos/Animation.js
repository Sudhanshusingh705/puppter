...
async function scrapeCollegeLogos() {
  ...
  let browser;
  let allLogos = [];
  const totalColleges = 580; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/animation-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'Animation_colleges_logos.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/animation-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
