...
async function scrapeCollegeLogos() {
  ...
  let browser;
  let allLogos = [];
  const totalColleges = 2280; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/pharmacy-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'Pharmacy_colleges_logos.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/pharmacy-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
