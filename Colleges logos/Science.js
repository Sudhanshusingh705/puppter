...
async function scrapeCollegeLogos() {
  ...
  let browser;
  let allLogos = [];
  const totalColleges = 6500; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/science-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'Science_colleges_logos.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/science-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
