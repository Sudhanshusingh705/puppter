...
async function scrapeCollegeLogos() {
  ...
  let browser;
  let allLogos = [];
  const totalColleges = 6300; // Total colleges to scrape
  ...
  try {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'Engineering_colleges_logos.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/engineering-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
