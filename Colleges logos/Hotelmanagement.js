...
async function scrapeCollegeLogos() {
  ...
  let browser;
  let allLogos = [];
  const totalColleges = 1200; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/hotel-management-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'HotelManagement_colleges_logos.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/hotel-management-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
