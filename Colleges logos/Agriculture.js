...
async function scrapeCollegeLogos() {
  ...
  let browser;
  let allLogos = [];
  const totalColleges = 990; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/agriculture-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'Agriculture_colleges_logos.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/agriculture-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
