...
async function scrapeCollegeLogos() {
  ...
  let browser;
  let allLogos = [];
  const totalColleges = 670; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/architecture-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'Architecture_colleges_logos.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/architecture-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
