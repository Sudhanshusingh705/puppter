...
async function scrapeCollegeLogos() {
  ...
  let browser;
  let allLogos = [];
  const totalColleges = 1420; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/vocational-courses-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'Vocational_colleges_logos.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/vocational-courses-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
