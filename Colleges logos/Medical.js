...
async function scrapeCollegeLogos() {
  ...
  let browser;
  let allLogos = [];
  const totalColleges = 2400; // Total colleges to scrape
  ...
  try {
    ...
    await page.goto('https://collegedunia.com/medical-colleges', {
    ...
    const csvWriter = createObjectCsvWriter({
      path: 'Medical_colleges_logos.csv',
      header: [
    ...
    while (scrapedCount < totalColleges && attempts < maxAttempts) {
      ...
      await page.goto(`https://collegedunia.com/medical-colleges?page=${pageNum}`, {
        waitUntil: 'networkidle2',
      });
